...
**编译器**无条件相信，然后编译执行主函数，发现Log了，就会说：好的，小兄弟，这个方案里面我知道你有这个`Log`，干啥的我不知道，但是其他的没什么问题，可以执行。让链接器去找找，老链啊，看看人力表里是不是有这个叫`Log`的，把它叫过来干活了。

**链接器**收到指令，开始在项目里找`Log`：`Log`，这活儿该你干了，过来干活！
此时`Log`函数接到任务，开始干自己的活，干完以后给个结果，然后程序继续执行。
